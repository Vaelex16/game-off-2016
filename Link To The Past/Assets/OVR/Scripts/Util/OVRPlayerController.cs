...
public class OVRPlayerController : MonoBehaviour
{
    ...
    public float weaponBobMin = 0.5f;
    public float weaponBobMax = 0.7f;
    public float bobRate = 1;
    public GameObject Weapons;
    private bool weaponBobUp = false;
    ...
    public void BobWeapons()
    {
        Vector3 weaponPosition = Weapons.GetComponent<Transform>().localPosition;
        //Debug.Log(weaponPosition);
        // Moving upwards
        if(weaponBobUp)
        {
            if (weaponPosition.y > weaponBobMax)
                weaponBobUp = false;
            else
            {
                weaponPosition.y += bobRate * Time.deltaTime;
                Vector3.Lerp(Weapons.GetComponent<Transform>().localPosition, weaponPosition, bobRate);
                
            }
        }
        else
        {
            if (weaponPosition.y < weaponBobMin)
                weaponBobUp = true;
            else
            {
                weaponPosition.y -= bobRate * Time.deltaTime;
        ...
        Weapons.transform.localPosition = weaponPosition;
    }
